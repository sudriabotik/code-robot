...
void conversion_string_to_function_code()
{
    if (serialus.info_a_traiter == true)
    {
        ...
        else if (check_string(0, "pump"))
        {
            serialus_traitement_pompes();
        }
    ...
    else if (check_string(1, "av"))
    {
        if (check_nb_param(3))
        {
            if (check_sont_des_nombres(2, serialus.buffer[2], serialus.buffer[3]))
            {
                ...
                if (check_coherence(d < 5000 && d > 0 && abs(v) < 150))
                {
                    init_clignotement();
...
{
    if (serialus.nb_param < 3)
    {
        print_manque_parametre();
        list_serialus(SER_ODOMETRIE);
    }
    else if (check_string(1, "entrax"))
    {
...
/**
 * Fonction qui traite toutes les frames relavtives au code "pump" pour les pompes à vide
 */
void serialus_traitement_pompes()
{
    if (serialus.nb_param < 1)
    {
        print_manque_parametre();
        list_serialus(SER_POMPE);
    }
    else if (check_string(1, "d"))
    {
        if (REGISTRE_MOTEUR_X)// == PWM_X_VALEUR_MAX)
            pompes(POMPE_DROITE, OFF);
        else
            pompes(POMPE_DROITE, ON);
    }
    else if (check_string(1, "g"))
    {
        if (ALIM_MOTEUR_Y)
