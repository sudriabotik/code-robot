...
void rejoindre (double x, double y, int8_t sens_marche, double pourcentage_vitesse)
{
    ...
    else if (erreur == BLOCAGE)
    {
        ...
        {
            if(compteur_rejoindre < 5)
            {
                compteur_rejoindre++;
                rejoindre(975, 76.65, MARCHE_AVANT, 40);             
                //printf("BLOCAGE - Nouvel essai");   
            }
            else
...
void avancer_reculer (double distance, double pourcentage_vitesse)
{
    ...
    if ( erreur == EVITEMENT)
    {
        if (EVITEMENT_ADV.mode == ACTION_EVITEMENT) //VS EVITEMENT_NORMAL ?
        {
            action_evitement();
        }
        else if (EVITEMENT_ADV.mode == EVITEMENT_NORMAL) //VS ACTION_EVITEMENT ?
        {
            printf("Evit");
            //action en cas d'évitements
        }
